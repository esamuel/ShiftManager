...
class WageCalculationService {
    ...
    func calculateWage(for shift: ShiftModel) async throws -> WageCalculation {
        ...
        // Determine day type
        let isWeekend = calendar.isDateInWeekend(shift.startTime)
        let isFestiveDay = await isFestiveDay(shift.startTime)
        let isSpecialDay = isWeekend || isFestiveDay || shift.isSpecialDay
        
        // Get applicable overtime rules
        let request = NSFetchRequest<OvertimeRule>(entityName: "OvertimeRule")
        request.predicate = NSPredicate(format: "isActive == %@", NSNumber(value: isSpecialDay))
